...
interface MQInterface
{
	...
	*/
	...
	["ami","amd"] int execCmd(string cmd,string sqlcode,string param,out string set,out string error);      //远程管理,用于执行SQL等


	//数据库相关接口
	["ami","amd"] idempotent int select(string sql,string param,out string set,out string error);    //远程查询数据库
	["ami","amd"] idempotent int selectCompress(string sql,string param,out string set,out string error);    //远程查询数据库
	...
	["ami","amd"] idempotent bool selectPrepareByParam(string sql,string param,out string sID,out string error);  
	["ami","amd"] idempotent bool selectPrepare(string sql,out string sID,out string error);
	["ami","amd"] idempotent int  selectNext(string sID,out string set);
    ...
    idempotent ByteSeq getFileCompressed(string path, int pos, int num,out int iResult);



	//上传文件
	 ["ami", "amd", "nonmutating", "cpp:const"] 
	int UploadFile(string sFile, int pos,int num,ByteSeq filecontent);
